...
const createTransactionWithSplit = async (req, res) => {
    try {
        const { title, amount, category, date, description, group, splitBetween } = req.body;
        ...
        return ApiResponse(201, "Transaction and split created successfully", { transaction, splitRecord }, res);
...
const updatePaymentStatus = async (req, res) => {
    try {
        ...
        if (!split) return res.status(404).json({ error: "Split record not found" });
        ...
        const transaction = new Transaction({
            ...
            amount: split.splitBetween.find(detail => detail.member.toString() === memberId).amount,
            date: new Date(),
        ...
        return ApiResponse(200, "Payment status updated successfully", { split }, res);
...
const getAllSplits = async (req, res) => {
    try {
        const splits = await Split.find({}).populate("expense").populate("splitPayer").populate("splitBetween.member", "name email phoneNo");
...
const getSplitById = async (req, res) => {
    try {
        ...
        const split = await Split.findById(splitId).populate("expense").populate("splitPayer").populate("splitBetween.member", "name email phoneNo");
